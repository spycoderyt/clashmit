...
test('regenerates five HP every four seconds, once per boundary',()=>{
 const p=player();assert.deepEqual(HEALTH_REGEN,{amount:5,intervalMs:4000,max:100});
 settle(p,4999);assert.equal(p.health,50);settle(p,5000);assert.equal(p.health,55);settle(p,5000);assert.equal(p.health,55);settle(p,9000);assert.equal(p.health,60);
});
...
test('lingering damage and healing agree for delayed and frequent updates',()=>{
 ...
 assert.equal(coarse.health,frequent.health);assert.equal(coarse.health,18);assert.equal(coarseDamage.reduce((n,d)=>n+d.amount,0),frequentDamage.reduce((n,d)=>n+d.amount,0));
...
test('only live rounds heal; new rounds reset time and late ticks stop at round end',()=>{
 ...
 room.phase='playing';room.endsAt=30000;settleRoom(room,29000);assert.equal(p.health,55);settleRoom(room,90000);assert.equal(p.health,55);
 room.phase='finished';settleRoom(room,100000);assert.equal(p.health,55);
 room.phase='playing';room.endsAt=200000;settleRoom(room,103999);assert.equal(p.health,55);settleRoom(room,104000);assert.equal(p.health,60);
});
