...
test('public standings never expose private state and reconnecting after leaving preserves identity',async t=>{
 ...
 const a=await enter();const response=await fetch(base+'/api/leaderboard'),body=await response.json();assert.equal(response.headers.get('cache-control'),'no-store');assert.equal(body.players[0].points,0);assert.equal(body.players[0].wins,0);assert.equal(body.rules.win,200);assert.deepEqual(Object.keys(body.players[0]).sort(),['avatar','bestStreak','coins','currentStreak','deaths','id','knockouts','name','points','rank','rounds','wins']);
