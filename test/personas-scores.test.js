...
test('a witch is paid for skeleton and poison damage, not only for what lands on impact',()=>{
 ...
 tick(room,army.resolvedAt+6000);assert.equal(b.health,70,'30 damage before the first 8-second regeneration tick');assert.equal(a.roundPoints,30*POINTS.damage,'all 30 skeleton damage is credited');
 const poison=land(room,'a','poison','b',army.resolvedAt+7000);tick(room,poison.resolvedAt+5000);
 assert.equal(b.health,55,'50 total damage offset by one regeneration tick');assert.equal(a.roundPoints,50*POINTS.damage,'5 on impact and 15 lingering');
});
