...
test('caster, victim and observer receive kill and bounty notices, with the real 90-coin reward',async t=>{
 ...
 for(let streak=1;streak<=6;streak++){
  const {events}=await kill(a,b);assert.ok(events.every(event=>event.coins===30));
  if(streak>=3){const notices=await Promise.all(clients.map(client=>client.next('arena-event',m=>m.kind==='streak'&&m.actorId===a.id&&m.text.includes(`${streak}${streak>=5?'x':''} killstreak`))));assert.equal(new Set(notices.map(event=>event.id)).size,1);if(streak>=5)assert.ok(notices.every(event=>event.text===`Streak Ace is on a ${streak}x killstreak. Bounty: triple the coins!`));}
 ...
 const {events,shot}=await kill(b,a);assert.ok(events.every(event=>event.coins===90&&event.balance===90));
 ...
 const live=await(await fetch(base+'/api/live')).json();assert.equal(live.players.find(p=>p.id===hunter.id).coins,90);assert.equal(live.kills.filter(k=>k.victim==='Streak Ace').length,1);assert.equal(live.kills[0].coins,90);
});
