import {createMeleeGame} from './melee-game.js';
...
const connection=createGameConnection({
 ...
 onMessage:m=>{
  ...
  if(m.type==='melee'){
   roundOverlay.impact(m);if(m.damage>0)hitSinceState.add(m.targetId);
   if(m.actorId===myId||m.targetId===myId){audio.play('arrows','impact');haptics.play(m.blocked?'tap':'hit');}
   if(m.actorId===myId)notify(m.blocked?'Shield blocked your sword':`Sword hit · ${m.damage/10} heart`);
   if(m.targetId===myId){notify(m.blocked?'Your shield blocked the sword':`Sword hit · −${m.damage/10} heart`);if(m.damage>0)$('arena').animate([{boxShadow:'inset 0 0 90px #ff303080'},{boxShadow:'inset 0 0 0 #ff303000'}],{duration:240});}
  }
...
function stopCamera(){melee.stop();damageFlash.clear();cameraEpoch++;faceTracker.stop();stream?.getTracks().forEach(t=>t.stop());stream=null;$('camera').srcObject=null;trackingStatus='Camera off';selected=null;lockId=null;targetOverlay.hide();}
...
$('leave').onclick=()=>{orbitalView.clear();melee.clear();clearFlights();targetOverlay.hide();focusId=null;selected=null;lockId=null;if(hudPreview){const p=me();if(p.health>0){p.health=0;p.diedAt=now();p.respawnAt=now()+10000;}roundOverlay.update(room,myId);renderState();}else if(joined)send({type:'retire'});};
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){melee.clear();voice.pause();faceTracker.reset();if(joined&&!practice){send({type:'inactive'});faceResent=false;connection.stop();}}else if(joined&&!practice){if(!faceScan.isOpen)voice.resume();connection.start();notify('Find your opponent again before casting.');}});
