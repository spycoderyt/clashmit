...
const minimap=createMinimap({container:$('arena'),send:message=>hudPreview?false:send(message),notify,...(hudPreview?{geolocation:previewGps}:{})});window.addEventListener('pagehide',()=>minimap.stop());
...
$('leave').onclick=()=>{orbitalView.clear();clearFlights();targetOverlay.hide();focusId=null;selected=null;lockId=null;if(hudPreview){const p=me();if(p.health>0){p.health=0;p.diedAt=now();p.respawnAt=now()+10000;}roundOverlay.update(room,myId);renderState();}else if(joined)send({type:'retire'});};
...
$('arena').classList.toggle('round-live',room.phase==='playing');const p=me();if(!practice)safeWrite('fieldspell-persona',p.nextPersona||p.persona||'mage');const displayHealth=trackingPractice?opponent()?.health:p.health;healthHud.update(room.players,myId,displayHealth,trackingPractice?'Target health':'Your health');$('room-label').textContent=hudPreview?'HUD PREVIEW':trackingPractice?'ONE PERSON FACE TEST':practice?'SOLO PRACTICE':(room.eventRound?.mode==='koth'?'KING OF THE HILL':'FFA');$('start-round').hidden=room.continuous||room.hostId!==myId;$('start-round').disabled=!trackingPractice&&room.phase==='playing';$('start-round').textContent=trackingPractice?'Reset target':room.phase==='finished'?'New round':'Start round';
 $('player-score').hidden=!p.score;const balance=String(p.score?.coins||0);if($('player-score').dataset.balance!==balance){$('player-score').dataset.balance=balance;$('player-score').replaceChildren();const coin=document.createElement('img');coin.src='/media/coin.svg';coin.className='coin-icon';coin.alt='Coins';$('player-score').append(coin,document.createTextNode(` ${balance}`));}renderInventory(p);
...
// Owned skills cast by voice. Locked skills can be bought from their cards.
...
function renderDeck(){
 ...
 for(const spell of deck){if(room.economy&&CONSUMABLES[spell])continue;const info=SPELL_INFO[spell],rule=room.economy?ruleFor(me(),spell):SPELLS[spell],name=room.economy?skillName(me(),spell):info.label,button=document.createElement(room.economy&&ATTACKS[spell]&&skillLevel(me(),spell)<2?'button':'div');button.id=spell;button.className='spell '+info.css;button.setAttribute('role','group');button.setAttribute('aria-label',`Say ${name} to cast`);
  if(room.economy&&ATTACKS[spell]&&!skillLevel(me(),spell)){const price=UNLOCK_COST[attacksFor(myPersona()).indexOf(spell)],cost=document.createElement('span'),coin=document.createElement('img');button.classList.add('skill-mystery');button.type='button';button.removeAttribute('role');button.setAttribute('aria-label',`Unlock skill ${attacksFor(myPersona()).indexOf(spell)+1} for ${price} coins`);cost.className='mystery-price';coin.src='/media/coin.svg';coin.alt='Coins';cost.append(coin,String(price));button.append(cost);button.onclick=()=>{const p=me(),quote=shopQuote(p.loadout,myPersona(),'unlock',spell);if(quote.error){notify(quote.error);return;}if((p.score?.coins||0)<quote.cost){notify(`You need ${quote.cost} coins to unlock this skill.`);return;}if(hudPreview){p.score.coins-=quote.cost;p.loadout.skills[spell]=1;renderState();}else send({type:'purchase',kind:'unlock',item:spell});};$('spells').append(button);continue;}
  ...
  button.classList.toggle('upgraded',skillLevel(me(),spell)>1);
...
function renderCombat(){
 ...
 for(const spell of myDeck()){
  const rule=room.economy?ruleFor(p,spell):SPELLS[spell],remaining=Math.max(0,(p.cooldowns[spell]||0)-at),button=$(spell);if(!button||button.classList.contains('skill-mystery'))continue;const cover=button.querySelector('.cooldown');cover.style.display=remaining?'flex':'none';cover.textContent=(remaining/1000).toFixed(1);
...
if(hudPreview){
 ...
 if(previewMode==='airstrike'){
  ...
  function previewStrike(caster=false,escape=false){controls.open=false;previewLocation={latitude:42.3601,longitude:-71.0942,accuracy:4};previewFix?.();const point={...previewLocation};orbitalView.clear();previewOrbital([{id:crypto.randomUUID(),actorId:caster?myId:'dummy',name:caster?'You':'Alex',point,radius:ORBITAL.radius,startsAt:now(),endsAt:now()+5000,victims:caster?[]:[{id:myId}]}],myId);if(escape)setTimeout(()=>{previewLocation={...previewLocation,latitude:42.3605};previewFix?.();},1300);}
