...
let socket,room,myId,practice=false,trackingPractice=false,leaving=false,joined=false,offset=0,retry=0,reconnectTimer;
...
const now=()=>Date.now()+offset,me=()=>room?.players.find(p=>p.id===myId),opponent=()=>room?.players.find(p=>p.id!==myId);
...
function finishShot(shot,tracked){if(!room)return;if(practice){const event=impactFireball(room,myId,shot.shotId,tracked);if(!event.error)handleImpact(event);if(opponent().health<=0){room.phase='finished';room.winners=[myId];}renderState();}else send({type:'impact',shotId:shot.shotId,tracked});}
function effect(spell,{shot,projectile=true}={}){
 clearTimeout(effectTimer);const layer=$('fx');layer.className='';layer.replaceChildren();const burst=document.createElement('div');burst.className='spell-burst';
 ...
 const target=targetPoint(shot?.targetId)||{x:.5,y:.4};
 let depth=false;
 if(spell==='fireball'&&projectile){
  const startedAt=performance.now(),roundEndsAt=room.endsAt,actor=myId,flight=createFlight({startedAt,flightMs:shot.flightMs||FLIGHT_MS});
  if(flights.has(shot.shotId))return;
  const timer=setInterval(()=>{const active=!!room&&room.endsAt===roundEndsAt&&myId===actor&&!document.hidden;const result=flight.step(performance.now(),simulated()||!!matchedPerson()?.fresh,active);if(result){clearInterval(timer);flights.delete(shot.shotId);if(!result.cancelled)finishShot(shot,result.tracked);}},40);flights.set(shot.shotId,timer);
  try{depth=!!fireScene?.fire({...target,getTarget:()=>targetPoint(shot.targetId),flightMs:shot.flightMs});}catch(e){console.warn('Fireball graphics fallback',e);}
  notify('Fireball launched');
 }
...
function showArena(){loadGraphics();$('lobby').hidden=true;$('arena').hidden=false;$('shirt-open').hidden=simulated();$('tracking-retry').hidden=simulated();$('camera-instructions').textContent=trackingPractice?'Use the front camera to track your own registered shirt.':practice?'Practice a 3D fireball over your camera with a simulated target.':'Register your shirt, then point the camera at your opponent.';$('camera-privacy').textContent='Camera frames stay on your phone. No location permissions needed.';$('camera-prompt').hidden=!!stream?.active;}
...
function connect(){
 ...
 socket.onmessage=e=>{let m;try{m=JSON.parse(e.data);}catch{return;}
  ...
  if(m.type==='spell'&&m.actorId===myId){castPending=false;effect(m.spell,{shot:m});}
 ...
 socket.onclose=e=>{clearTimeout(timer);if(leaving||practice)return;$('connection').textContent='Disconnected · casting paused';selected=null;fireScene?.clear();if(e.code===4000){notify('This player was opened in another tab.');return;}if(joined)reconnectTimer=setTimeout(connect,Math.min(5000,800*2**retry++));else $('join').disabled=false;};
}
...
function beginPractice(realTracking=false){trackingPractice=realTracking;practice=true;leaving=true;socket?.close();myId='self';offset=0;const make=(id,name)=>({id,name,health:100,shieldUntil:0,cooldowns:{},connected:true});room={phase:'playing',hostId:myId,endsAt:Date.now()+180000,winners:[],players:[make(myId,$('name').value.trim()||'You'),make('dummy','Practice target')]};showArena();$('connection').textContent=trackingPractice?'Local shirt test · no server':'Solo · simulated target';renderState();if(trackingPractice){room.players[1].name='Your shirt';shirtCamera.open();}else startCamera();}
...
function stopSensors(){for(const timer of flights.values())clearInterval(timer);flights.clear();stopCamera();shirtCamera.stop();fireScene?.clear();clearTimeout(effectTimer);$('fx').className='';$('fx').replaceChildren();voice.stop();}
$('leave').onclick=()=>{leaving=true;clearTimeout(reconnectTimer);if(!practice)send({type:'leave'});socket?.close();sessionStorage.removeItem('fieldspell-token');$('arena').hidden=true;stopSensors();room=null;myId=null;practice=false;trackingPractice=false;joined=false;rosterSignature='';$('lobby').hidden=false;$('camera-prompt').hidden=false;$('boxes').replaceChildren();$('join-status').textContent='One shared arena · 2 players';};
...
const shirtCamera=setupShirtCamera({beforeOpen:()=>{$('shirt-help').textContent=trackingPractice?'This local test recognizes your sampled shirt. No second player is needed.':'Choose a different color from your opponent. No photo is saved or uploaded.';voice.stop();stopCamera();fireScene?.clear();},onSave:profile=>{if(trackingPractice){opponent().shirt=profile;identityTrack.reset();renderState();notify('Shirt saved. Keep your upper body in view.');}else send({type:'shirt',profile});},onClose:()=>{if(!$('arena').hidden)startCamera();}});
$('shirt-open').onclick=()=>{if(!practice&&room?.phase==='playing'){notify('Wait until the round ends to change your shirt.');return;}shirtCamera.open();};
...
function renderState(){if(!me())return;$('arena').classList.toggle('round-live',room.phase==='playing');const p=me(),displayHealth=trackingPractice?opponent()?.health:p.health;$('health-title').textContent=trackingPractice?'TARGET HEALTH':'YOUR HEALTH';$('health-value').innerHTML=`${displayHealth} <small>/ 100</small>`;$('health-fill').style.width=displayHealth+'%';$('room-label').textContent=trackingPractice?'ONE PERSON SHIRT TEST':practice?'SOLO PRACTICE':'TWO PLAYER ARENA';$('start-round').hidden=room.hostId!==myId;$('start-round').disabled=!trackingPractice&&room.phase==='playing';$('start-round').textContent=trackingPractice?'Reset target':room.phase==='finished'?'New round':'Start round';$('shirt-open').textContent=(trackingPractice?opponent()?.shirt:p.shirt)?'Retake shirt':'Register shirt';$('shirt-open').disabled=!practice&&room.phase==='playing';
...
function renderAim(){
 ...
 $('target-status').textContent=simulated()?'Simulated target':!(trackingPractice?opponent()?.shirt:me()?.shirt)?'Tap Register shirt to get ready.':!opponent()?.shirt?'Waiting for your opponent’s shirt sample.':!practice&&similarity(me().shirt,opponent().shirt)>.8?'Shirts too similar · use different colors.':!stream?.active?'Enable your camera.':match?(locked?`${opponent().name} locked · cast Fireball`:trackingPractice?'Keep your shirt inside the reticle':'Aim the reticle at your opponent'):trackingPractice?'Show your shirt · step back if only your face is visible.':'Find your opponent’s shirt · move closer if needed';
...
function cast(spell){if(!room)return;if(room.phase!=='playing'){notify('The host needs to start the round first.');return;}renderAim();if(spell==='fireball'&&(!selected||Date.now()-lockSince<200||(!simulated()&&!matchedPerson()?.fresh))){notify('Aim at your opponent until their shirt locks.');return;}if(practice){const event=spell==='fireball'?launchFireball(room,myId,selected,crypto.randomUUID()):castSpell(room,myId,spell,selected);if(event.error){notify(event.error);return;}effect(spell,{shot:event});renderState();}else if(socket?.readyState===WebSocket.OPEN){if(castPending)return;castPending=true;const request=++castRequest;send({type:'cast',spell,targetId:selected});notify('Casting '+spell+'…');setTimeout(()=>{if(castPending&&request===castRequest){castPending=false;notify('Cast not confirmed. Check the connection.');}},2500);}else notify('Reconnecting. Casting is paused.');}
for(const spell of Object.keys(SPELLS))$(spell).onclick=()=>cast(spell);
$('start-round').onclick=()=>{if(practice){fireScene?.clear();if(trackingPractice){opponent().health=100;room.phase='playing';room.endsAt=Date.now()+180000;room.winners=[];room.shots=[];me().cooldowns={};renderState();}else beginPractice(false);return;}send({type:'start'});};
...
setInterval(()=>{if(!room)return;renderAim();for(const spell of Object.keys(SPELLS)){const remaining=Math.max(0,(me()?.cooldowns[spell]||0)-now()),cover=$(spell).querySelector('.cooldown');cover.style.display=remaining?'flex':'none';cover.textContent=(remaining/1000).toFixed(1);if(spell!=='fireball')$(spell).disabled=room.phase!=='playing'||me()?.health<=0||(!practice&&socket?.readyState!==WebSocket.OPEN);}
 if(room.phase==='playing'&&trackingPractice){$('phase').textContent='Real shirt tracking · local test';}else if(room.phase==='playing'){const seconds=Math.max(0,Math.ceil((room.endsAt-now())/1000));$('phase').textContent=`${Math.floor(seconds/60)}:${String(seconds%60).padStart(2,'0')} remaining`;if(practice&&seconds===0){room.phase='finished';room.winners=[];renderState();}}
...
if(new URLSearchParams(location.search).get('test')==='shirt')beginPractice(true);
