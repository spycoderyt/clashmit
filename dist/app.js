...
import {coverRect} from './shirt.js?v=headband1';
import {bandColor} from './headband.js?v=headband1';
import {createTargetTrack,aimContains} from './target-track.js?v=headband1';
import {createFlight} from './projectile-flight.js?v=headband1';
import {createPersonTracker} from './detection.js?v=headband1';
import {setupShirtCamera} from './shirt-camera.js?v=headband1';
...
function loadGraphics(){graphicsLoading??=import('./fireball.js?v=headband1').then(m=>{fireScene=m.createFireballRenderer($('arena'));}).catch(()=>{fireScene=null;});return graphicsLoading;}
...
function renderAim(){
 ...
 $('target-status').textContent=simulated()?'Simulated target':!bandColor((trackingPractice?opponent()?.shirt:me()?.shirt)?.rgb)?'Tap Scan headband to get ready.':!opponent()?.shirt?'Waiting for your opponent’s headband sample.':!practice&&bandColor(me().shirt.rgb)===bandColor(opponent().shirt.rgb)?'Headbands too similar · use different colors.':!stream?.active?'Enable your camera.':match?(locked?`${opponent().name} locked · cast Fireball`:trackingPractice?'Aim at the person wearing your headband':'Aim the reticle at your opponent'):trackingPractice?'Show your headband and upper body.':'Find your opponent’s headband · move closer if needed';
