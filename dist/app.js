...
let stream,watchId=null,position=null,heading=null,headingAt=0,orientationActive=false,upright=true,selected=null,recognition,listening=false;
...
function stopSensors(){stream?.getTracks().forEach(t=>t.stop());stream=null;$('camera').srcObject=null;if(watchId!==null)navigator.geolocation.clearWatch(watchId);watchId=null;position=null;heading=null;headingAt=0;window.removeEventListener('deviceorientation',onOrientation);window.removeEventListener('deviceorientationabsolute',onOrientation);orientationActive=false;recognition?.abort();}
...
if(!Speech){$('voice').disabled=true;$('voice').textContent='Use spell buttons';}else{
 $('voice').onclick=()=>{if(listening){recognition?.stop();return;}recognition=new Speech();recognition.lang='en-US';recognition.interimResults=false;recognition.continuous=false;
 recognition.onstart=()=>{listening=true;$('voice').textContent='Listening… say Fireball, Shield or Heal';$('voice').classList.add('listening');};
 recognition.onresult=e=>{const words=e.results[0][0].transcript.toLowerCase();const spell=/\bfire\s?ball\b/.test(words)?'fireball':/\bshield\b/.test(words)?'shield':/\bheal\b/.test(words)?'heal':null;if(spell)cast(spell);else notify('Say “Fireball”, “Shield”, or “Heal”.');};
 recognition.onerror=()=>notify('Voice unavailable. Use the spell buttons; Safari may require Siri enabled.');
 recognition.onend=()=>{listening=false;$('voice').classList.remove('listening');$('voice').textContent='◎ Say a spell';};
 try{recognition.start();}catch{notify('Could not start voice input. Use a spell button.');}
 };
}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){recognition?.abort();headingAt=0;}else if(joined)notify('Check your GPS and compass before casting.');});
