...
import {loadFaceEngine,detectRegion,describe} from './face-engine.js?v=face8';
...
self.onmessage=async({data})=>{
 ...
 try{
  for(const [index,region] of data.regions.entries()){
   const bitmap=region.bitmap,k=region.width/bitmap.width,found=await detectRegion(bitmap,{x:0,y:0,width:bitmap.width,height:bitmap.height},data.minScore??.5,region.detectSize||640);
  ...
  for(const face of wanted)face.descriptor=await describe(data.regions[face.index].bitmap,face.local);
  postMessage({type:'faces',seq:data.seq,ms:performance.now()-started,faces:faces.map(({box,score,landmarks,pixels,descriptor})=>({box,score,landmarks,pixels,descriptor:descriptor||null}))});
