...
const MAX_GAP_MS=250,GESTURE_MS=220,MIN_TRAVEL=.12,MIN_STEP=.012,MIN_SPEED=.8;
...
export function createMeleeTracker({damage=5,cooldown=1000}={}){
 ...
 let previous,lastAt,lastHitAt,history;
 function reset(){previous=null;lastAt=null;lastHitAt=-Infinity;history=[];}
 reset();
 const remaining=at=>Math.max(0,cooldown-(at-lastHitAt));
 const result=(at,hit=null)=>({hit,phase:remaining(at)>0?'cooldown':'ready',cooldownRemaining:remaining(at)});
 function loseHistory(){previous=null;history=[];}
 function update({hand=null,faces=[],at}={}){
  ...
  lastAt=at;
  // A reacquired hand starts a new path; never bridge missing or invalid frames.
  if(!validHand(hand)){loseHistory();return result(at);}
  if(previous&&at-previous.at>MAX_GAP_MS)loseHistory();
  const current={x:hand.x,y:hand.y,size:hand.size,at},from=previous;
  previous=current;
  // Keep a short motion window, not accumulated jitter or a long slow drag.
  while(history.length>1&&history[1].at<at-GESTURE_MS)history.shift();
  history.push(current);
  if(!from||remaining(at)>0)return result(at);
  const step=Math.hypot(current.x-from.x,current.y-from.y),speed=step/((at-from.at)/1000);
  const travel=Math.max(...history.map(p=>Math.hypot(current.x-p.x,current.y-p.y)));
  if(step<MIN_STEP||speed<MIN_SPEED||travel<MIN_TRAVEL)return result(at);
