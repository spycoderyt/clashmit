...
export function createTargetOverlay(arena,container){
 ...
 frame.className='person-box band-target';label.className='person-label band-target-label';meter.min=0;meter.max=100;meter.low=30;meter.high=60;meter.optimum=100;meter.setAttribute('aria-label','Opponent health');label.append(title,meter,info);container.append(frame,label);
 ...
 function update(target,player,{shielded=false,simulated=false,poisoned=false,healed=false,accent='',piercer='Lightning'}={}){
  ...
  text(title,(player.score?`#${player.score.rank} `:'')+(player.name||'Target'));if(meter.value!==player.health){meter.value=player.health;meter.style.setProperty('--health-color',healthColor(player.health));}meter.hidden=!!target.pending;
