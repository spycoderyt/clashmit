...
export function createTargetOverlay(arena,container){
 ...
 function update(target,player,{shielded=false,simulated=false}={}){
  ...
  text(title,player.name||'Target');if(meter.value!==player.health)meter.value=player.health;
  text(info,shielded?'◇ Shield · lightning pierces':simulated?'Simulated':target.confirmed?(target.fresh?'Headband locked':'Tracking…'):'Acquiring headband…');
 }
