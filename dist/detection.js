import {mapBand} from './headband.js?v=smooth1';
import {findPairBands} from './headband-pair.js?v=pair1';
...
export function createPersonTracker(video,onResult,onStatus,getProfiles){
 ...
 async function frame(){
  ...
  if(!profiles.opponent?.id){onResult([],sourceWidth,sourceHeight,frameAt);onStatus('Scan a two-color headband first');timer=setTimeout(frame,100);return;}
  const key=JSON.stringify([profiles.opponent.id,profiles.own?.id]);if(key!==profileKey){lastFull=0;profileKey=key;}
  ...
  if(!worker){deliver(findPairBands(ctx.getImageData(0,0,w,h),profiles.opponent,profiles.own,buffers));return;}
  try{const bitmap=await createImageBitmap(canvas);if(g!==generation){bitmap.close();return;}timeout=setTimeout(()=>{worker?.terminate();worker=null;onStatus('Using camera color tracking');frame();},2000);worker.postMessage({type:'frame',bitmap,width:w,height:h,opponent:{id:profiles.opponent.id},own:{id:profiles.own?.id??null}},[bitmap]);}catch{if(g===generation){worker?.terminate();worker=null;timer=setTimeout(frame,0);}}
 }
 function start(){stop();active=true;lastFull=0;onStatus('Starting headband tracking…');
  ...
  const g=generation;worker=new Worker(new URL('./color-worker.js?v=pair1',import.meta.url),{type:'module'});
