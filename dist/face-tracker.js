...
import {createFaceTracks} from './face-tracks.js?v=face14';
import {startFaceEngine,detectFaces,grabRegion} from './face-client.js?v=face13';
import {createFaceSearch,detectionTime} from './face-search.js?v=1';
...
export function createFaceTracker(video,{getGallery,onStatus=()=>{},focus={x:.5,y:.4}}){
 const tracks=createFaceTracks(),search=createFaceSearch();
 let active=false,generation=0,timer=null,width=0,height=0,lastFrameAt=0,bodyWorker=null,bodyReady=false,bodyBusy=false,lastBodies=0,bodySeq=0;
 ...
 async function tick(){
  ...
  width=video.videoWidth;height=video.videoHeight;const point={x:focus.x*width,y:focus.y*height},regions=search.next({width,height,at:started,point,follow:tracks.regions(started,2,point)});
  try{
   ...
   if(named.length)startBodyWorker();
 ...
 return{
  ...
  stop(){active=false;generation++;clearTimeout(timer);tracks.reset();lastFrameAt=0;search.reset();bodyBusy=false;},
