...
export function setupLeaderboard({root,lobby,url,getMyId}){
 ...
 async function refresh(){
  ...
  try{
   ...
   const data=await response.json(),players=data.players.filter(p=>(p.bestStreak||0)>0),myId=getMyId(),signature=JSON.stringify([players,myId]);
   if(signature!==last){last=signature;list.replaceChildren(...players.map(p=>{
    ...
    for(const value of [rankLabel(p.rank),p.name+(p.id===myId?' (you)':''),(p.bestStreak||0).toLocaleString(),p.knockouts||0,p.deaths||0]){const cell=document.createElement('td');cell.textContent=value;row.append(cell);}
    return row;
   }));}
   hasData=true;status.textContent=players.length?'Ranked by best kill streak':'No kills yet. Start a streak.';
