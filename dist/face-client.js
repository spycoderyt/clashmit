...
export function startFaceEngine(onProgress){
 ...
 ready??=new Promise((resolve,reject)=>{
  try{worker=new Worker(new URL('./face-worker.js?v=face8',import.meta.url),{type:'module'});}catch(e){reject(e);return;}
...
export async function detectFaces(source,regions,options={}){
 ...
 const prepared=[];for(const r of regions)prepared.push({x:r.x,y:r.y,width:r.width,height:r.height,detectSize:r.detectSize,full:!!r.full,bitmap:await grabRegion(source,r,r.maxSize||640)});
