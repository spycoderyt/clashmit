...
export function createGameServer({maxPlayers=null,maxBufferedBytes=256*1024,countdownMs=5000,scoreFile=null}={}){
 ...
 function view(room){const board=scores.standings(),byId=new Map(board.map(p=>[p.id,p]));const now=Date.now();for(const p of room.players)replenishMana(p,now);return {maxPlayers,code:room.code,hostId:room.hostId,phase:room.phase,startsAt:room.startsAt||0,endsAt:room.endsAt,winners:room.winners,results:room.results||null,players:room.players.map(({token,socket,disconnectedAt,...p})=>({...p,score:byId.get(p.id)})),shots:room.shots||[],combat:{mana:MANA,spells:SPELLS},serverTime:now};}
   ...
   try{
    ...
    if(m.type==='join'){
     ...
     if(player){if(player.socket!==ws){clients.delete(player.socket);player.socket.close(4000,'Opened on another connection');}player.socket=ws;player.connected=true;player.disconnectedAt=null;}
      ...
      player={...identity,health:100,mana:MANA.max,manaUpdatedAt:Date.now(),shieldUntil:0,cooldowns:{},connected:true,shirt:null,socket:ws};room.players.push(player);if(!room.players.some(p=>p.id===room.hostId&&p.connected))room.hostId=player.id;
     }
    ...
    if(m.type==='start'){
     ...
     finish(room);const shot=room.shots?.find(s=>s.shotId===m.shotId&&s.actorId===player.id),before=room.players.find(p=>p.id===shot?.targetId)?.health;
     const event=impactProjectile(room,player.id,m.shotId,m.tracked===true);if(!event.error){recordScore(room,event,before);finish(room);broadcast(room,event);}
