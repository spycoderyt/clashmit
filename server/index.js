...
import {COINS_PER_KILL,MAX_HEALTH,CONSUMABLES} from '../dist/economy.js';
...
export function createGameServer({maxPlayers=null,maxBufferedBytes=256*1024,countdownMs=5000,scoreFile=null,continuous=false,economy=continuous,enhanced=continuous,adminPassword=process.env.ADMIN_PASSWORD||'',respawnDelayMs=RESPAWN_MS}={}){
 ...
 const recordKill=(room,event)=>{
  const {actorId,targetId,...publicEvent}=event;const at=Date.now(),actor=room.players.find(p=>p.id===actorId),target=room.players.find(p=>p.id===targetId);
  ...
  const cheers=['Huzzah!','Unstoppable!','What a streak!','Keep it going!'];if(event.streak>=3)announce(room,{kind:'streak',actorId,text:`${event.killer} is on a ${event.streak} killstreak. ${cheers[event.streak%cheers.length]}`});
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/api/live'&&['GET','HEAD'].includes(req.method)){
   ...
   res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(req.method==='HEAD'?undefined:JSON.stringify({players,kills,events,round:rounds.snapshot(rooms.get('ARENA')),online:online.size,serverTime:Date.now()}));
  }
 ...
 function deactivate(room,player){if(!player.connected)return;rememberOrbitalLocation(room,player);rounds.account(room,Date.now());player.connected=false;player.disconnectedAt=Date.now();player.location=null;player.faceReady=false;if(room.economy&&(room.eventRound?.mode||'ffa')!=='ffa'&&room.phase==='playing'){if(!(player.actionLockUntil>Date.now()))player.health=0;player.eliminated=true;if(room.eventRound?.players[player.id])room.eventRound.players[player.id].eliminated=true;}if(room.faces)delete room.faces[player.id];if(room.avatars)delete room.avatars[player.id];for(const p of room.players)if(p.connected){send(p.socket,{type:'faces',faces:{[player.id]:null}});send(p.socket,{type:'avatars',avatars:{[player.id]:null}});}rounds.tick(room);}
 function damageEvent(room,event){if(event.amount>0){const actor=room.players.find(p=>p.id===event.actorId);if(actor)send(actor.socket,{type:'damage',actorId:event.actorId,targetId:event.targetId,amount:event.amount});}}
