...
export function creditContinuous(room,store,{actorId,targetId,amount,lethal=false,actorLife},onKill){
 ...
 if(knockout){const score=store.standings().find(p=>p.id===actorId);onKill?.({killer:actor.name,victim:target.name,streak:countStreak?score.currentStreak:0});if(countStreak&&score.currentStreak>=3)return{type:'killstreak',actorId,name:actor.name,streak:score.currentStreak};}
}
export function scoreContinuousHit(room,store,event,before,onKill){
 ...
 return creditContinuous(room,store,{actorId:event.actorId,targetId:target.id,amount:Math.max(0,before-target.health),lethal:target.health<=0,actorLife:event.actorLife},onKill);
}
